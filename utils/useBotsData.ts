...
// Préchargement côté client
let clientPreloadPromise: Promise<void> | null = null;

async function preloadClientData(page: number, limit: number) {
  ...
  clientPreloadPromise = (async () => {
    try {
      ...
      if (response.ok) {
        ...
        const cacheKey = `bots-${page}-${limit}`;
      ...
      console.warn('Erreur lors du préchargement côté client:', error);
    }
  })();
...
export function useBotsData({ 
  ...
  const fetchData = useCallback(async (forceRefresh = false) => {
    ...
    // Si on a des données en cache mais qu'elles sont un peu anciennes, les utiliser temporairement
    if (!forceRefresh && cached && now - cached.timestamp < CACHE_TTL + STALE_TTL) {
      setData(cached.data);
      setIsStale(true);
      setLoading(false);
      setError(null);
    ...
    try {
      ...
      const timeoutId = setTimeout(() => controller.abort(), 8000); // Timeout augmenté pour le premier chargement
  ...
  // Préchargement automatique au montage du composant
  useEffect(() => {
    // Précharger les données de la page suivante
    const nextPage = page + 1;
    const nextCacheKey = `bots-${nextPage}-${limit}`;
    
    if (!globalCache.has(nextCacheKey)) {
      preloadClientData(nextPage, limit).catch(console.error);
    }
  }, [page, limit]);
